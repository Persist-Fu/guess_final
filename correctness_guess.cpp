...
int main() // MPI: main 函数需要接收 argc 和 argv
{
    ...
    if(rank!=size-1)
    {
        ...
        if (rank == 0) {
            // 1. 发送最后一批剩余的密码
            if (!q.guesses.empty()) {
                //cout << "Rank 0: Sending final batch of " << q.guesses.size() << " guesses." << endl;
                int data_type_id = STRING_DATA;
                MPI_Send(&data_type_id, 1, MPI_INT, size - 1, 0, MPI_COMM_WORLD);
                send_vector_of_strings(q.guesses, size - 1, MPI_COMM_WORLD);
                q.guesses.clear();
            ...
            // 2. 发送最终的终止信号
            //cout << "Rank 0: Sending final termination signal." << endl;
            int data_type_id = BOOL_DATA;
            bool final_signal = false;
            MPI_Send(&data_type_id, 1, MPI_INT, size - 1, 0, MPI_COMM_WORLD);
            MPI_Send(&final_signal, 1, MPI_CXX_BOOL, size - 1, 0, MPI_COMM_WORLD);
        }
            */
    }
    ...
    if(rank==size-1)
    {
        //cout<<"aiaiaiiaiaiiaiiaiianiu"<<endl;
        ...
        while(continue_next||!(q_guesses.empty()))
        {
            ...
            for (size_t i = 0; i < total; i += 4) {
                ...
                for (size_t j = 0; j < batch_size; ++j) 
                {
                    if(test_set.find(q_guesses[i+j])!=test_set.end())
                    {
                        cracked+=1;
                    }
					batch[j] = q_guesses[i + j];
