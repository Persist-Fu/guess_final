...
using namespace std;
#pragma omp declare reduction (merge:vector<string>:\
    omp_out.insert(omp_out.end(),make_move_iterator(omp_in.begin()),make_move_iterator(omp_in.end())))\
    initializer(omp_priv = std::vector<std::string>())
class segment
{
public:
